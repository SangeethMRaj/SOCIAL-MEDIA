...
function ForgotPassword() {
    ...
    const submit = ()=>{
        console.log('submit');
        const {email,currentpassword,password} = user
        if(email){
            console.log('in axios');
            axios.post("http://localhost:4000/forgotpassword",user).then((response) =>{
          ...
          <form onSubmit={handleSubmit(submit)}>
              <div>
                {error && <h1 className='mx-28 mb-4 text-red-600'>Invalid Email Address</h1>}
                  ...
                  <input type="email" name="email"  id="email" {...register("email",{required:'Email is required'})} onChange={handleChange} class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name@company.com" required=""/>
